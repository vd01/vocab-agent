...
test.describe("Word card", () => {
	...
	test("pronunciation button is visible on word card", async ({ page }) => {
		...
		const pronounceButton = page
			.locator('button[title*="发音"], button[aria-label*="发音"]')
			.first();
	...
	test("FSRS rating buttons are visible after flip", async ({ page }) => {
		...
		if (await wordCard.isVisible().catch(() => false)) {
			...
			const ratingButtons = page
				.locator("button")
				.filter({ hasText: /Again|Hard|Good|Easy/ });
	...
	test("review session shows new/review count badges", async ({ page }) => {
		...
		// If no review words available, the command may show "没有待复习" instead
		if (!hasNew && !hasReview) {
			const noReviewMsg = page.getByText(/没有待复习|暂无/).first();
			const hasNoReview = await noReviewMsg.isVisible().catch(() => false);
			// Either badges or no-review message should be present
			expect(hasNoReview).toBeTruthy();
		} else {
	...
	test("word card shows flip instructions", async ({ page }) => {
		...
		const flipInstruction = page
			.getByText("点击或按空格键翻转查看释义")
			.first();
