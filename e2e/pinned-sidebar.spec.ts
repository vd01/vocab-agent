...
test.describe("Pinned sidebar", () => {
	...
	test("shows empty state when no pinned words", async ({ page }) => {
		...
		// Should show empty state message - use .first() to avoid strict mode (2 elements match)
		const emptyState = page.getByText("暂无置顶单词").first();
	...
	test("pin cards have hover actions", async ({ page }) => {
		...
		if (await pinCards.isVisible().catch(() => false)) {
			...
			const hoverActions = page
				.locator('[class*="opacity-0 group-hover:opacity-100"]')
				.first();
