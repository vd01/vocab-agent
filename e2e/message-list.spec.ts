...
test.describe('Message list', () => {
  ...
  test('user messages are right-aligned', async ({ page }) => {
    ...
    const msgWrapper = page.locator('div.justify-end').filter({ hasText: 'RightAlignTest' }).first();
    await expect(msgWrapper).toBeVisible();
  });

  test('assistant avatar is visible for assistant messages', async ({ page }) => {
    const input = page.getByPlaceholder('输入消息或 / 命令...');
    ...
    const avatar = page.locator('svg').filter({ has: page.locator('path[d*="M9.813 15.904"]') }).first();
...
test.describe('Chat input', () => {
  ...
  test('input placeholder changes in dev mode', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.getByPlaceholder('描述你想添加或修改的功能...')).toBeVisible();
    } catch {
  ...
  test('shows keyboard shortcut hint', async ({ page }) => {
    await expect(page.getByText('按 Enter 发送，Shift+Enter 换行')).toBeVisible();
  });
});
