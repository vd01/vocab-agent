...
test.describe('UI Components', () => {
  ...
  test('cards have correct styling', async ({ page }) => {
    // Send a command that might show cards
    const input = page.getByPlaceholder('输入消息或 / 命令...');
    ...
    const cards = page.locator('[class*="rounded-lg"], [class*="rounded-xl"], [class*="rounded-2xl"]').first();
  ...
  test('separator renders correctly', async ({ page }) => {
    ...
    const separators = page.locator('hr, [class*="separator"], [role="separator"]').first();
  ...
  test('progress indicators render correctly', async ({ page }) => {
    ...
    const loadingIndicator = page.locator('text=/思考中|加载中|Loading/').first();
    try {
