...
export const TEST_CASES: TestCase[] = [
  ...
  },
  {
    ...
    expectation: '先用 file-read 读取文件确认当前内容和行号，然后用 <<<file-edit:generated/tools/hello.js:replace:N-M>>> 标记块替换对应行。不会尝试调用不存在的 file-edit 工具。',
  ...
  },
  {
    ...
    expectation: 'create-command 返回错误，提示与内置命令冲突。Agent 应自动换一个名称（如 /my-stats 或 /learn-stats），重新注册并告知用户新命令名。',
  ...
  },
  {
    ...
    expectation: '高复杂度任务，涉及：1. file-list 了解目录结构 2. file-read 查阅文档 3. <<<file-write>>> 写入 toolCode 4. <<<file-write>>> 写入组件代码 5. create-command 注册 6. test-command 测试 7. 如果测试失败，根据 _errorDetail 修复 8. save-lesson 保存经验（如果有价值发现）',
  ...
  },
  {
    ...
    expectation: '1. 先用 db-query（custom SQL: SELECT）确认有多少 source="test" 的单词\n2. 如果没有，通过 toolCode 沙盒添加测试数据\n3. 因为 db-query 的 custom 模式只支持 SELECT，写操作需要通过动态命令的 toolCode 沙盒执行（用 register-tool 或 create-command 注册临时命令，再用 test-command 执行）\n4. 不默认注册长期命令——执行完清理操作后告知用户结果',
