...
export const TEST_CASES: TestCase[] = [
  ...
  {
    id: 'TC-06',
    title: 'readSeek_grep — 浏览项目结构',
    category: '单工具调用',
    complexity: 'simple',
    prompt: '看看 generated 目录下都有什么文件',
    expectation: '使用 readSeek_grep 或 bash 工具列出 generated 目录结构。返回目录内容。',
    coveragePoint: 'readSeek_grep/bash 工具，项目文件浏览',
    group: 'A',
    timeoutMs: 60_000,
  },
  ...
  // ── Group B: 标记块文件操作链（TC-07→08→09 有依赖）──────────────────
  {
    ...
    expectation: '使用 readSeek_write 或 write 工具写入文件 generated/tools/hello.js，内容是一个 async 函数，返回 { type: \'message\', message: \'Hello from developer!\' }。',
    coveragePoint: 'readSeek_write/write 工具文件写入，确认 Agent 使用 pi 内置工具写文件',
    group: 'B',
    timeoutMs: 60_000,
  },
  {
    ...
    expectation: '编辑已有文件时，必须先用 readSeek_read 或 read 工具读取文件确认内容和行号，然后使用 readSeek_edit 工具替换对应行。行号应基于读取的实际内容，而非猜测。',
    coveragePoint: 'readSeek_read + readSeek_edit 工具，编辑前先读取的规范',
    group: 'B',
    dependsOn: ['TC-07'],
    timeoutMs: 120_000,
  },
  {
    ...
    expectation: '先用 readSeek_read 读取文件确认行号，然后用 readSeek_edit 插入注释。',
    coveragePoint: 'readSeek_edit 插入操作，行号定位',
    group: 'B',
    dependsOn: ['TC-08'],
    timeoutMs: 120_000,
  },
  ...
  },
  {
    ...
    expectation: '1. 用 readSeek_write 或 write 工具写入 toolCode 到 generated/tools/word-stats.js（查询数据库，按首字母聚合）\n2. 用 readSeek_write 或 write 工具写入组件代码到 generated/components/word-stats-panel.tsx（含 \'use client\'、export default、Tailwind 样式）\n3. 调用 create-command，同时传入 toolCodePath 和 componentCodePath\n4. 调用 test-command 验证',
  ...
  },
  {
    ...
    expectation: 'Agent 编写 toolCode 时如果 create-command 返回 syntax-error，应根据错误信息定位并修复，使用 readSeek_edit 或 readSeek_write 覆盖原文件后重新调用 create-command。',
  ...
  },
  {
    ...
    expectation: '核心流程：1. readSeek_write 写入 toolCode（查询数据库，随机出 5 个单词和 5 个释义）2. readSeek_write 写入交互式组件代码（点击配对游戏 UI）3. create-command 注册 4. test-command 测试 5. 如果测试失败，根据 _errorDetail 修复。可选步骤：readSeek_read 查阅文档（如不确定 API 时）、save-lesson（如有价值发现）。',
...
export const TEST_GROUPS = [
  ...
  { name: 'B', label: '文件操作链 (pi SDK)', needsCleanup: true },
