...
export async function POST(req: Request) {
	...
	if (!action || !word) {
		return Response.json(
			{ type: "error", message: "请提供 action 和 word 参数" },
			{ status: 400 },
		);
	}
	...
	switch (action) {
		...
		default:
			return Response.json(
				{ type: "error", message: `未知操作: ${action}` },
				{ status: 400 },
			);
	}
}
...
async function addAndPin(word: string, side: "left" | "right", group?: string) {
	...
	const sidePins = await db
		...
		.from(pinnedWords)
		.where(
			sql`${pinnedWords.side} = ${side} AND ${pinnedWords.archivedAt} IS NULL`,
		)
	...
	if (sidePins.length >= MAX_PINS_PER_SIDE) {
		...
		const otherPins = await db
			...
			.from(pinnedWords)
			.where(
				sql`${pinnedWords.side} = ${otherSide} AND ${pinnedWords.archivedAt} IS NULL`,
			)
			.orderBy(asc(pinnedWords.position));
	...
	const maxPos =
		sidePins.length > 0 ? Math.max(...sidePins.map((p) => p.position)) : -1;
...
async function addToGroup(word: string, groupId?: string, groupName?: string) {
	...
	const existingMember = await db
		...
		.where(
			sql`${wordGroupMembers.groupId} = ${groupId} AND ${wordGroupMembers.wordId} = ${wordId}`,
		)
		.limit(1);
