				...
				: Array.isArray(m.content)
					? m.content.slice(0, 5).map((c) => {
							...
							if (c.type === "tool-result")
								return { type: "tool-result", toolCallId: c.toolCallId };
							if (c.type === "tool-call")
								return { type: "tool-call", toolName: c.toolName };
							return { type: c.type };
						})
					: "[complex]",
	}));
...
export async function POST(req: Request) {
	try {
		...
		const {
			mode: rawMode,
			modeSwitched: rawModeSwitched,
			activeGroup,
		} = body as { mode?: string; modeSwitched?: boolean; activeGroup?: string };
		...
		const filteredMessages =
			contextManager.filterFunctionalMessages(sanitizedMessages);
		const filteredCount = sanitizedMessages.length - filteredMessages.length;
		if (filteredCount > 0) {
			console.log(
				`[Context Manager] Filtered ${filteredCount} functional command messages`,
			);
		}
		...
		const debugId = DEBUG_ENABLED
			? `dbg-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
			: "";

		// Snapshot the actual messages sent to LLM (before any step mutates them)
		const requestMessagesSnapshot = DEBUG_ENABLED
			? serializeMessagesForDebug(trimmedMessages)
			: [];

		// Track timing for tokens/s calculation
			...
			onStepFinish: async (stepResult) => {
				try {
					...
					if (agentType === "developer" && stepResult.text) {
						...
						if (blockCount > 0) {
							console.log(
								`[Chat API] Parsed ${blockCount} file block(s) from step ${stepCount}`,
							);
						}
					}
					...
					if (DEBUG_ENABLED) {
						...
						const stepLog = {
							...
							agent: agentType,
							model:
								stepResult.model?.modelId ?? config.model.modelId ?? "unknown",
							request: {
								instructions:
									finalInstructions.slice(0, 500) +
									(finalInstructions.length > 500 ? "..." : ""),
			...
			onEnd: async () => {
				...
				try {
					...
					if (debugLogs.length > 0) {
						setDebugLogs(debugId, debugLogs);
						console.log(
							`[Chat API] Debug logs written via onEnd: ${debugId} (${debugLogs.length} steps)`,
						);
					}
				} catch (err) {
					console.error(
						"[Chat API] Failed to capture debug logs via onEnd:",
						err,
					);
				}
			},
			...
			async start(controller) {
				...
				try {
					while (true) {
						...
						// Decode chunk and scan for SSE events
						...
						while ((boundary = buffer.indexOf("\n\n")) !== -1) {
							events.push(buffer.slice(0, boundary + 2));
							buffer = buffer.slice(boundary + 2);
						}

						for (const event of events) {
							// Detect the 'finish' SSE event to inject token usage after it
							if (!finishReceived && event.includes('"type":"finish"')) {
								finishReceived = true;

								// Forward the finish event first
								controller.enqueue(encoder.encode(event));

								// Inject token usage as message-metadata event (debug panel only)
								if (DEBUG_ENABLED) {
									try {
										const steps = await result.steps;
										let aggInput = 0;
										...
										for (const step of steps) {
											if (step.usage) {
												aggInput += step.usage.inputTokens ?? 0;
												aggOutput += step.usage.outputTokens ?? 0;
											}
										}
										const finalInput = Math.max(aggInput, totalInputTokens);
										const finalOutput = Math.max(aggOutput, totalOutputTokens);
										const elapsedMs = Date.now() - streamStartTime;
										const tokensPerSec =
											elapsedMs > 0 && finalOutput > 0
												? (finalOutput / (elapsedMs / 1000)).toFixed(1)
												: "-";

										const usageMetaEvent = `data: ${JSON.stringify({
										...
										controller.enqueue(encoder.encode(usageMetaEvent));
										console.log(
											`[Chat API] Token usage: in=${finalInput}, out=${finalOutput}, ${tokensPerSec} tok/s, ${elapsedMs}ms`,
										);
									} catch (err) {
										console.error(
											"[Chat API] Failed to inject token usage event:",
											err,
										);
									}
								}
					...
					if (agentType === "developer") {
						const { fileBlockStore } = await import(
							"@/lib/ai/utils/file-block-store"
						);
						...
						if (stepText) {
							...
							if (parsed > 0) {
								console.log(
									`[Chat API] Parsed ${parsed} file block(s) from step text at stream end`,
								);
							}
							fileBlockStore.clearStepText();
						}
						...
						if (fileBlockStore.size > 0) {
							const { executeFileBlocks } = await import(
								"@/lib/ai/utils/file-block-executor"
							);
							...
							if (results.length > 0) {
								const resultText = results
									...
									.join("");
								// Use a valid text-delta format with id and delta fields
								const textDeltaEvent = `data: ${JSON.stringify({ type: "text-delta", id: "file-block-result", delta: resultText })}\n\n`;
								controller.enqueue(encoder.encode(textDeltaEvent));
								console.log(
									`[Chat API] Executed ${results.length} file block(s) at stream end`,
								);
							}
						}
					}
					...
					if (stepCount >= MAX_STEPS) {
						...
						// Use a valid text-delta format with id and delta fields
						const textDeltaEvent = `data: ${JSON.stringify({ type: "text-delta", id: "step-limit-warning", delta: warning })}\n\n`;
						controller.enqueue(encoder.encode(textDeltaEvent));
						console.log(
...
function sanitizeUIMessages(messages: UIMessage[]): UIMessage[] {
	return messages.map((msg) => {
		...
		if (hasToolResult && !hasToolCall) {
			const filteredParts = (normalized.parts as PartLike[]).filter(
				(p) => !isToolPart(p),
			);
			return {
				...normalized,
				...
				parts: (filteredParts.length > 0
					? filteredParts
					: [{ type: "text" as const, text: "" }]) as any,
			};
		}
