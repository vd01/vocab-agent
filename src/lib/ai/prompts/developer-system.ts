...
**所有文件写入和编辑都通过标记块协议完成**，代码不需要 JSON 转义，原样写在标记块中即可。

### ⚠️ 关键：分两步操作

**标记块和 tool 调用必须在不同的消息中**：

1. **第一条消息**：只输出标记块文本（包含代码），**不调用任何 tool**
2. **第二条消息**：调用对应的 tool 确认操作（只传文件路径等元信息）

**绝对不要在同一条消息中同时输出标记块和调用 tool！** 否则 tool 找不到标记块内容。

### 写入文件（file-write）

\`\`\`
第一条消息：输出标记块（不调用 tool）
...
第二条消息：调用 tool 确认写入
file-write({ filePath: "generated/components/demo.tsx" })
\`\`\`

### 编辑文件 — 插入（file-edit insert）

在指定行号后插入代码（行号从 1 开始，insert:0 表示在文件开头插入）：
...
第二条消息：调用 tool 确认编辑
file-edit({ filePath: "generated/components/demo.tsx", mode: "insert", startLine: 4 })
\`\`\`

### 编辑文件 — 替换（file-edit replace）

替换指定行范围的代码（包含起始行和结束行）：
...
第二条消息：调用 tool 确认编辑
file-edit({ filePath: "generated/components/demo.tsx", mode: "replace", startLine: 6, endLine: 8 })
\`\`\`

### 编辑前先读取

编辑文件前，**必须先用 file-read 读取文件**，确认当前行号和内容，再决定 insert/replace 的行号。

### 标记块规则

- 标记块中的代码**原样写入文件**，不需要 JSON 转义（引号、反斜杠、换行都不需要额外处理）
- 每个标记块必须以 \`<<<end>>>\` 结束
- 一个回复中可以包含多个标记块（不同文件或同一文件的不同操作）
...
\`\`\`
消息1：输出 toolCode 标记块（不调用 tool）
<<<file-write:generated/tools/word-match.js>>>
async (args) => { ... }
<<<end>>>

消息2：输出组件代码标记块（不调用 tool）
<<<file-write:generated/components/word-match-panel.tsx>>>
import React...
<<<end>>>

消息3：确认写入 + 注册 + 测试
file-write({ filePath: "generated/tools/word-match.js" })
file-write({ filePath: "generated/components/word-match-panel.tsx" })
create-command({ name: "word-match", description: "连连看", toolCodePath: "generated/tools/word-match.js", componentCodePath: "generated/components/word-match-panel.tsx" })
test-command("word-match")
\`\`\`

**注意：标记块必须在 tool 调用之前的消息中输出。可以在一条消息中输出多个标记块，然后在下一条消息中批量确认写入。**
...
- **file-write**: 确认写入文件。先在回复中用 <<<file-write:路径>>>...<<<end>>> 输出代码，再调用本工具确认。可写目录: generated/、src/components/generated/、src/app/api/
- **file-read**: 读取现有代码（使用相对路径，如 "generated/components/demo.tsx"）。**编辑文件前必须先读取，确认行号。**
- **file-edit**: 确认编辑文件。先在回复中用标记块输出代码，再调用本工具确认。支持 insert（在指定行后插入）和 replace（替换指定行范围）两种模式。可编辑目录同 file-write。
