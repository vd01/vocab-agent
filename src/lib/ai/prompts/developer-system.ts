...
查询数据库的正确写法:
  ...
  const dueReviews = await db.select().from(tables.reviews).where(eq(tables.reviews.wordId, 'xxx'));
...
- 不要在函数内 import 模块，沙盒已注入 db、tables、fsrs
...
每次完成开发任务后，你必须主动总结经验教训并调用 save-lesson 保存。这是强制要求，不是可选项。
...
**保存什么：**
- **pattern（成功模式）**：验证有效的做法，如 "先写文件再引用路径，避免 JSON 转义问题"
- **anti-pattern（应避免的做法）**：导致问题的做法，如 "在 tool 参数中直接嵌入长代码"
- **tip（实用技巧）**：提高效率的技巧，如 "查询 reviews 表时用 eq 而不是手动过滤"
- **pitfall（常见陷阱）**：容易踩的坑，如 "组件名用 kebab-case 但 import 变量名用 PascalCase"
