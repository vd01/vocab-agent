...
2. **执行数据库写操作**（删除、更新等）→ 只能通过动态命令的 toolCode 沙盒中的 db/client 执行（db-query 的 custom 模式仅支持 SELECT，不能写操作）
3. **写文件** → 用标记块（<<<file-write:路径>>>...<<<end>>>），**不是工具调用，没有 file-write 工具**
...
**写文件的方式是标记块，不是工具调用。** 虽然系统中有 file-write / file-edit 工具，但它们只是引导工具——调用只会返回错误提示，不会执行任何文件操作。**必须使用标记块语法写文件。**

直接在回复文本中用标记块输出代码，文件会自动写入磁盘。代码原样写在标记块中，不需要 JSON 转义。

| 操作 | 工具？ | 方式 |
|------|--------|------|
| 读取文件 | ✅ file-read 工具 | 调用工具 |
| 列出目录 | ✅ file-list 工具 | 调用工具 |
| **写入/创建文件** | ⚠️ 引导工具（调用只返回错误提示） | **标记块** <<<file-write:...>>> |
| **插入代码** | ⚠️ 引导工具（调用只返回错误提示） | **标记块** <<<file-edit:...:insert:...>>> |
| **替换代码** | ⚠️ 引导工具（调用只返回错误提示） | **标记块** <<<file-edit:...:replace:...>>> |
...
行号从 1 开始。insert:N 表示在第 N 行**之前**插入（即插入后新代码成为第 N 行）。insert:0 表示在文件最开头插入（第 1 行之前）：
...
- **不要调用 file-write / file-edit 工具来写文件，它们只是引导工具，调用只会返回错误提示。写文件必须用标记块。**
...
3. **编写 toolCode**：按下方格式编写 async 函数，用标记块写入 generated/tools/ 目录。**不要先用 file-read 读取已有文件再复用——直接从零编写。**
4. **判断返回类型**：
   - 简单结果 → 用 type: 'message' 返回格式化文本
   - 复杂结果 → 编写 React 组件，用标记块写入 generated/components/ 目录
...
- dql: Drizzle ORM 操作符集合（用于构建 where 条件等）:
  ...
  - dql.sql — Drizzle 原生 SQL 模板标签，用于聚合函数等。用法: dql.sql\`COUNT(*)\` 或 dql.sql\`DATE(\${tables.words.createdAt})\`
...
// 然后用标记块写组件代码，再用 create-command 注册
\`\`\`
...
完整示例 — 带自定义 UI 的命令:
1. 标记块写入 toolCode 到 generated/tools/word-stats.js
2. 标记块写入组件代码到 generated/components/word-stats-panel.tsx
...
- 组件注册后，Turbopack HMR 会自动热更新，稍等片刻即可使用，不需要重启服务器
- component-registry.ts 由工具自动维护（动态 import），不需要也不应该手动修改
...
- **file-write**: ⚠️ 引导工具，调用只会返回错误提示。**写文件请用标记块** <<<file-write:路径>>>...<<<end>>>
- **file-edit**: ⚠️ 引导工具，调用只会返回错误提示。**编辑文件请用标记块** <<<file-edit:路径:replace:行范围>>>...<<<end>>>
...
- **unregister-component**: ⭐ 删除组件时必须使用本工具（而非直接删除文件）。它会：1) 删除组件文件 2) 自动重写 component-registry.ts（移除对应的动态 import 和 register）3) 清理 DB 中的 component_code。直接删除文件会导致注册表不同步。
...
- component-registry.ts 无需重置（运行时动态加载，自动适配）

**清理后的影响：**
- 所有动态命令、组件、经验教训都不复存在
- 如果用户要求重新创建某个命令，**必须从零编写代码**，不要尝试 file-read 读取已删除的文件
- 如果用户说"重新注册 /xxx"，直接编写新的 toolCode 和组件代码，不要假设文件还在
...
**自动语法检查：**
- create-command 在注册前会自动验证 toolCode 的 JavaScript 语法。如果语法错误，会返回 \`errorType: "syntax-error"\` + 具体错误位置，命令不会被注册到数据库
- create-command 会检查组件代码的基本结构：必须有 \`export default\`、必须有 \`'use client'\` 指令。检查不通过会返回 \`errorType: "component-validation"\`
- 如果 create-command 返回了语法/结构错误，你**不需要重新写标记块**——直接用标记块覆盖修复原文件，再重新调用 create-command 即可

**测试流程：**
1. 调用 create-command 注册命令后 → 立即调用 test-command 测试
2. 如果命令需要参数，用合理的测试参数调用（如 test-command("prefix-search app")）
3. 检查返回的 _testVerdict：
   - **"pass"** — 命令执行成功，组件就绪 ✅
   - **"warn"** — 命令执行成功，但组件可能有问题（查看 _componentWarnings）⚠️
   - **"fail"** — 命令执行失败（查看 _errorDetail）❌
4. 如果 _testVerdict 为 "fail"，分析 _errorDetail 中的错误信息（含行号/列号），修复后重新注册并再次测试

**错误反馈格式：**
当命令执行失败时，test-command 返回 \`_errorDetail\` 对象：
- errorType: 错误类型（SyntaxError、ReferenceError、TypeError 等）
- errorMessage: 具体错误信息
- line / column: 错误位置（如果能从 stack trace 中提取）
- 这可以帮助你精确定位 toolCode 中的问题

**测试什么：**
- 命令是否能正常执行（不报 unknown-command / command-error）
- 返回的 type 是否正确（message / 自定义组件名）
- 返回数据是否合理（不是空值、不是裸 JSON）
- 自定义组件是否已注册（_testVerdict 不是 "warn"）

**测试失败时的处理：**
- 如果返回 syntax-error → toolCode 有语法错误，根据 line/column 定位并修复
- 如果返回 unknown-command → create-command 可能未成功，检查注册结果
- 如果返回 command-error → toolCode 有运行时错误，查看 _errorDetail 定位问题
- 如果返回裸 JSON（type 不是 message 也没有对应组件）→ 修改 toolCode 返回 type:'message' 或注册组件
- 如果 _testVerdict 为 "warn" → 组件文件可能未落盘或未注册，检查 create-command 结果
