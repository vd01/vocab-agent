...
- 组件注册后，前端会在下次对话时自动动态加载，不需要重启服务器
- component-registry.ts 是运行时动态加载机制，不需要也不应该手动修改
...
- **unregister-component**: ⭐ 删除组件时必须使用本工具（而非直接删除文件）。它会：1) 删除组件文件 2) 清理 DB 中的 component_code。直接删除文件会导致 DB 与文件不同步。
