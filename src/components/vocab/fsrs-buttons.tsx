...
export function FsrsButtons({ wordId, onRate, pendingRating, disabled = false }: FsrsButtonsProps) {
  return (
    <div className="grid grid-cols-4 gap-2 mt-3 min-w-[260px] sm:min-w-[500px]">
      {RATING_CONFIG.map(({ value, label, color, disabledColor, icon }) => (
        <Button
          key={value}
          size="sm"
          disabled={disabled}
          className={`flex-1 min-w-0 py-3 h-auto ${disabled ? disabledColor + ' cursor-not-allowed' : color} ${!disabled && pendingRating === value ? 'ring-2 ring-offset-2 ring-primary' : ''}`}
          onClick={(e) => {
            e.stopPropagation();
