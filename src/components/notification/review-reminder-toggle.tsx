...
import { ReviewScheduler, type SchedulerConfig, DEFAULT_SCHEDULER_CONFIG } from '@/lib/notification/review-scheduler';
...
export function ReviewReminderToggle({ onReviewFromNotification, dueCount }: ReviewReminderToggleProps) {
  ...
  useEffect(() => {
    ...
    nm.init().then(() => {
      ...
      fetch('/api/settings?prefix=review.')
        ...
        .then(data => {
          ...
          setDailyReviewLimit(parseInt(s['review.dailyReviewLimit'] ?? '0', 10) || 0);
        })
        .catch(() => {});
    });
    ...
    window.addEventListener('review-notification-click', handleNotificationClick);

    return () => {
      unsub();
      window.removeEventListener('review-notification-click', handleNotificationClick);
    };
  }, [onReviewFromNotification]);
  ...
  const handleDailyNewLimitChange = useCallback(async (limit: number) => {
    ...
    try {
      await fetch('/api/settings', {
        ...
        body: JSON.stringify({ settings: { 'review.dailyNewLimit': String(limit) } }),
      });
    } catch {}
  }, []);
  ...
  const handleDailyReviewLimitChange = useCallback(async (limit: number) => {
    ...
    try {
      await fetch('/api/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ settings: { 'review.dailyReviewLimit': String(limit) } }),
      });
    } catch {}
  }, []);
  ...
  const canEnable = !isUnsupported && !isDenied;
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <select
                  ...
                  onChange={(e) => handleQuietStartChange(parseInt(e.target.value, 10))}
                  className="h-7 rounded border bg-background px-2 text-xs"
                >
                ...
                <select
                  ...
                  onChange={(e) => handleQuietEndChange(parseInt(e.target.value, 10))}
                  className="h-7 rounded border bg-background px-2 text-xs"
                >
                  {Array.from({ length: 24 }, (_, i) => (
                    <option key={i} value={i}>{String(i).padStart(2, '0')}:00</option>
            ...
            <div className="pt-2 border-t">
              ...
              <div className="flex items-center justify-between">
                ...
                <select
                  value={dailyNewLimit}
                  onChange={(e) => handleDailyNewLimitChange(parseInt(e.target.value, 10))}
                  className="h-7 rounded border bg-background px-2 text-xs"
                >
              ...
              <div className="flex items-center justify-between mt-1.5">
                ...
                <select
                  ...
                  onChange={(e) => handleDailyReviewLimitChange(parseInt(e.target.value, 10))}
                  className="h-7 rounded border bg-background px-2 text-xs"
                >
                  <option value={0}>不限</option>
                  <option value={20}>20</option>
