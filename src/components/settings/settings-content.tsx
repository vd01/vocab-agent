...
export function SettingsContent() {
	...
	// ── Web-compatible state (settings API) ──
	...
	// ── Web settings load/save ──
	async function loadWebSettings() {
		try {
			const r = await fetch("/api/settings?prefix=");
			const data = await r.json();
			const s = data.settings ?? {};
			setNotifEnabled(s["notification.enabled"] === "true");
			setNotifInterval(
	...
	async function saveWebSettings() {
		...
		try {
			await fetch("/api/settings", {
				...
				body: JSON.stringify({
					settings: {
						"notification.enabled": String(notifEnabled),
						"notification.intervalMinutes": String(notifInterval),
						"review.dailyNewLimit": String(dailyNewLimit),
						"review.dailyReviewLimit": String(dailyReviewLimit),
					},
				}),
			});
		} finally {
	...
	// ── Electron config load/save ──
	async function loadElectronConfig() {
	...
	async function handleNotificationChange(
		key: "reviewReminder" | "reminderInterval",
	...
	async function handleCloseToTrayChange(value: boolean) {
		if (!window.electronAPI || !config) return;
		const newConfig = await window.electronAPI.setConfig({
			window: { ...config.window, closeToTray: value },
		} as Partial<AppConfig>);
		setConfigState(newConfig);
	}
	...
	return (
		<div
			className="min-h-screen bg-background p-6 max-w-2xl mx-auto"
		...
		>
			...
			<div className="space-y-6">
				{/* ── Web-compatible: Notification settings ── */}
				<Card>
					<CardHeader>
							...
							<>
								<div className="flex items-center justify-between">
									...
									<Switch
										...
										onCheckedChange={(v) =>
											handleNotificationChange("reviewReminder", v)
										}
									/>
								</div>
				...
				{/* ── Web-compatible: Daily limits ── */}
				<Card>
					<CardHeader>
						<CardTitle className="text-base">每日学习限额</CardTitle>
						<CardDescription>
							每日新词和复习数量上限（0 = 不限制）
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="flex items-center justify-between">
							...
							<Input
								...
								value={dailyNewLimit}
								onChange={(e) =>
									setDailyNewLimit(parseInt(e.target.value) || 0)
								}
								min={0}
								className="w-24"
							/>
									...
									<Input
										...
										onChange={(e) => {
											setConfigState({
												...config,
												local: {
													port: parseInt(e.target.value) || 3088,
												},
											});
										}}
										min={1024}
				...
				{/* ── Electron-only: AI config ── */}
				{isElectron && config && config.mode === "local" && (
					<Card>
						<CardHeader>
							<CardTitle className="text-base">AI 服务配置</CardTitle>
							<CardDescription>
								配置 API 密钥和模型参数，修改后需重启本地服务生效
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="space-y-2">
							...
							<div className="grid grid-cols-2 gap-3">
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) => {
											setEnvTeacherModel(e.target.value);
											setEnvDirty(true);
										}}
								...
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) => {
											setEnvDeveloperModel(e.target.value);
											setEnvDirty(true);
										}}
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) => {
										setEnvAuthPassword(e.target.value);
										setEnvDirty(true);
									}}
				...
				<div className="flex justify-end">
					{isElectron ? (
						...
						<Button onClick={saveWebSettings} disabled={webSaving}>
							{webSaving ? "保存中..." : "保存设置"}
						</Button>
					)}
				</div>
